...
void MipsGenerator::visitFunction(IrFunction* func) {
    ...
    for (auto bb : func->blocks) {
        for (auto instr : bb->instructions) {
            if (!instr->type->isVoid()) {
                ...
                localStart += size;
                if (localStart % align != 0) localStart += (align - (localStart % align));
    ...
    if (localStart % 8 != 0) localStart += 4;
...
void MipsGenerator::visitInstr(Instr* instr) {
    switch (instr->instrType) {
        ...
        case InstrType::SREM: {
            ...
            std::string op;
            switch (instr->instrType) {
                case InstrType::ADD: op = "addu"; break;
                case InstrType::SUB: op = "subu"; break;
                case InstrType::MUL: op = "mul"; break;
                case InstrType::SDIV: op = "div"; break; 
                case InstrType::SREM: op = "div"; break; 
                default: break;
            }
        ...
        case InstrType::BR: {
            loadToRegister(instr->getOperand(0), T0);
            emit("bne " + T0 + ", $zero, " + getLabelName(dynamic_cast<IrBasicBlock*>(instr->getOperand(1))));
            emit("j " + getLabelName(dynamic_cast<IrBasicBlock*>(instr->getOperand(2))));
            break;
        }
        case InstrType::JUMP: {
            emit("j " + getLabelName(dynamic_cast<IrBasicBlock*>(instr->getOperand(0))));
            break;
        }
        case InstrType::CALL: {
            ...
            int stackArgs = 0;
            if (argCount > 4) stackArgs = argCount - 4;
            if (stackArgs > 0) {
...
int MipsGenerator::getSize(IrType* type) {
    if (type->isInt32()) return 4;
    if (type->isInt1()) return 4;
    if (type->isInt8()) return 1;
...
std::string MipsGenerator::getFunctionName(IrFunction* func) {
    if (func->isBuiltin) return func->name.substr(1);
