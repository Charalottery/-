...
void IRGenerator::visitConstDecl(ASTNode* node) {
    for (const auto& child : node->children) {
        if (child->name != "ConstDef") continue;
        ...
        if (dims.empty()) {
            ...
            for (size_t k = 0; k < dims.size(); ++k) {
                 int s = 1;
                 for (size_t j = k + 1; j < dims.size(); ++j) s *= dims[j];
                 strides.push_back(s);
                 if (k == 0) totalSize = dims[0] * s;
            }

            if (isGlobal) {
                 std::vector<IrConstant*> flatVals;
                 getGlobalInitVals(initVal, flatVals);
                 while(flatVals.size() < (size_t)totalSize) flatVals.push_back(IrConstantInt::get(0));
                 ...
                 std::function<IrConstant*(IrType*)> reconstruct = [&](IrType* t) -> IrConstant* {
                     if (t->isInt32()) return flatVals[offset++];
                     if (t->isArray()) {
                         IrArrayType* at = (IrArrayType*)t;
                         std::vector<IrConstant*> elms;
                         for(int i=0; i<at->numElements; ++i) elms.push_back(reconstruct(at->elementType));
...
void IRGenerator::visitVarDecl(ASTNode* node) {
    ...
    for (const auto& child : node->children) {
        ...
        if (isGlobal || isStatic) {
            ...
            if (!init) {
                if (dims.empty()) init = IrConstantInt::get(0);
                else {
                    // Zero init recursively
                    std::function<IrConstant*(IrType*)> makeZero = [&](IrType* t) -> IrConstant* {
                        if (t->isInt32()) return IrConstantInt::get(0);
                        if (t->isArray()) {
                            IrArrayType* at = (IrArrayType*)t;
                            std::vector<IrConstant*> elms;
                            for(int i=0; i<at->numElements; ++i) elms.push_back(makeZero(at->elementType));
                            return new IrConstantArray(t, elms);
                        }
                        return nullptr;
                    };
...
void IRGenerator::visitFuncDef(ASTNode* node) {
    ...
    for (const auto& pType : sym->paramTypes) {
        if (pType == "Int") paramTypes.push_back(IrBaseType::getInt32());
        else if (pType == "IntArray") paramTypes.push_back(new IrPointerType(IrBaseType::getInt32()));
        else paramTypes.push_back(IrBaseType::getInt32());
    }
    ...
    for (const auto& child : node->children) {
        if (child->name == "FuncFParams") {
            for (const auto& param : child->children) {
                if (param->name != "FuncFParam") continue; // Skip commas
    ...
    if (entry->instructions.empty() || entry->instructions.back()->instrType != InstrType::RET) {
        if (IrBuilder::currentBlock->instructions.empty() || IrBuilder::currentBlock->instructions.back()->instrType != InstrType::RET) {
             if (retType->isVoid()) IrBuilder::insertInstr(new ReturnInstr(nullptr));
             else IrBuilder::insertInstr(new ReturnInstr(IrConstantInt::get(0)));
        }
    }
...
void IRGenerator::visitStmt(ASTNode* node) {
    if (node->children[0]->name == "LVal") {
        ...
        if (format.size() >= 2) format = format.substr(1, format.size() - 2);
        ...
        for (size_t i = 0; i < format.size(); ++i) {
            if (format[i] == '%' && i + 1 < format.size()) {
                ...
                if (format[i+1] == 'n') charCode = 10;
                else if (format[i+1] == 't') charCode = 9;
                else if (format[i+1] == '\"') charCode = 34;
                else if (format[i+1] == '\\') charCode = 92;
                else if (format[i+1] == '0') charCode = 0;
...
IrValue* IRGenerator::visitMulExp(ASTNode* node) {
    if (node->children.size() == 1) {
        ...
        std::string op = node->children[1]->token.value;
        InstrType type;
        if (op == "*") type = InstrType::MUL;
        else if (op == "/") type = InstrType::SDIV;
        else type = InstrType::SREM;
...
IrValue* IRGenerator::visitRelExp(ASTNode* node) {
    ...
    std::string op = node->children[1]->token.value;
    IcmpCond cond;
    if (op == "<") cond = IcmpCond::SLT;
    else if (op == ">") cond = IcmpCond::SGT;
    else if (op == "<=") cond = IcmpCond::SLE;
    else cond = IcmpCond::SGE;
...
int IRGenerator::evaluateConstExp(ASTNode* node) {
    if (!node) return 0;
    if (node->name == "ConstExp" || node->name == "Exp") return evaluateConstExp(node->children[0].get());
    
    if (node->name == "AddExp") {
        if (node->children.size() == 1) return evaluateConstExp(node->children[0].get());
        int lhs = evaluateConstExp(node->children[0].get());
        int rhs = evaluateConstExp(node->children[2].get());
        if (node->children[1]->token.value == "+") return lhs + rhs;
        else return lhs - rhs;
    }
    ...
    if (node->name == "UnaryExp") {
        if (node->children[0]->name == "PrimaryExp") return evaluateConstExp(node->children[0].get());
    ...
    if (node->name == "PrimaryExp") {
        if (node->children[0]->name == "LVal") return evaluateConstExp(node->children[0].get());
